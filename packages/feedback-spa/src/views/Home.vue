  ...
  <div>
    <div class="home pf-u-mt-4xl" v-if="!loading">
      <div class="pf-l-grid pf-m-gutter">
        <div
          class="pf-l-grid__item pf-m-4-col"
          v-for="(stat, index) in feedbackStats"
          v-bind:key="stat.id"
        >
          <div
            class="pf-c-card click-card"
            id="card-action"
            v-on:click="setCategory(stat.title, index)"
            v-bind:class="{ active: index === selectedCategoryIndex }"
          >
      ...
      <div class="pf-l-grid pf-m-gutter pf-u-mt-xl">
        <div class="pf-u-display-flex pf-m-8-col">
          <div v-for="(state, index) in feedbackStates" v-bind:key="state.id">
            <div
              class="status-box"
              v-on:click="setState(state.name, index)"
              v-bind:class="{ active: index === selectedStateIndex }"
            >
              {{ state.name }}
              <span
                class="badge op-badge"
                v-bind:class="{
                  'op-open': state.name === 'Open',
                  'op-close': state.name === 'Close',
                }"
                v-if="state.name !== 'All'"
                >{{ state.count }}</span
              >
          ...
          <div class="pf-u-ml-md pf-c-dropdown pf-m-expanded">
            <button
              ...
              aria-expanded="true"
              v-on:click="enableToogle = !enableToogle"
            >
            ...
            >
              <li>
                <a
                  ...
                  v-on:click="
                    selectedModule = 'Any';
                    enableToogle = !enableToogle;
                  "
                  >Any</a
                >
              </li>
              <li
                v-for="(module, index) in uniqueModuleList(allFeedback)"
                v-bind:key="index"
              >
                <a
                  class="pf-c-dropdown__menu-item"
        ...
        <div class="pf-l-grid__item pf-m-1-col">
          <button
            ...
            type="button"
            v-on:click="csvExport(allFeedback)"
          >
      ...
      <div class="pf-u-mt-xl">
        ...
        >
          <ListFeedback
            v-if="recordSize !== 0"
            :feedback="feedback"
            @openModal="openDetailsModal(feedback)"
          />
        </span>
        ...
        <div class="pf-c-pagination pf-m-compact" v-if="recordSize !== 0">
          <div class="pf-c-options-menu">
            <div
              class="pf-c-options-menu__toggle pf-m-text pf-m-plain pf-u-m-sm"
            >
            ...
            <select
              class="pf-c-form-control page-width"
              id="perPage"
              name="perPage"
              aria-label="per page entry"
              v-model="pageSize"
            >
          ...
          <nav class="pf-c-pagination__nav" aria-label="Pagination">
            <div class="pf-c-pagination__nav-control pf-m-prev">
              <button
                ...
                type="button"
                :disabled="Number(pageNumber) < Number(pageSize)"
                aria-label="Go to previous page"
                v-on:click="pageNumber = Number(pageNumber) - Number(pageSize)"
              >
        ...
        >
          ...
          <header class="pf-c-modal-box__header">
            <h1 class="pf-c-modal-box__title" id="modal-md-title">
              ...
              <a
                class="link-color"
                :href="`${selectedFeedback.ticketUrl}`"
                target="_blank"
              >
                Issue -
                {{
                  selectedFeedback.ticketUrl.split("/")[
                    selectedFeedback.ticketUrl.split("/").length - 1
                  ]
                }}</a
              >
            </h1>
          ...
          <div class="pf-c-modal-box__body" id="modal-md-description">
            ...
            <div class="pf-u-mt-sm pf-u-display-flex pf-l-grid pf-m-gutter">
              <span class="pf-c-label">
                <span class="pf-c-label__content">{{
                  selectedFeedback.module.toUpperCase()
                }}</span>
              </span>

              <span class="pf-c-label pf-m-red">
                <span class="pf-c-label__content">{{
                  selectedFeedback.state.toUpperCase()
                }}</span>
              </span>
            </div>
            ...
            <div class="pf-u-mt-sm pf-l-grid pf-m-gutter">
              <div class="pf-m-6-col">
                ...
                <p>
                  <a
                    v-bind:href="`mailto:${selectedFeedback.createdBy.uid}@redhat.com`"
                    >{{ selectedFeedback.createdBy.name || "N/A" }}</a
                  >
                </p>
              </div>
              ...
              <div class="pf-m-6-col">
                ...
                <p>
                  <a
                    v-bind:href="`mailto:${selectedFeedback.assignee.email}`"
                    >{{ selectedFeedback.assignee.name || "N/A" }}</a
                  >
                </p>
              </div>
            </div>
            ...
            <div
              class="pf-u-mt-sm pf-l-grid pf-m-gutter"
              v-if="selectedFeedback.experience"
            >
...
export default {
  ...
  async created() {
    this.allFeedback = await this.$apollo
      .query({
        query: ListFeedbacks,
      })
      .then((response) => {
    ...
    this.feedbackStats = [
      ...
      {
        id: 2,
        count: this.allFeedback.filter(
          (feedback) => feedback.category === "FEEDBACK"
        ).length,
        title: "Feedback",
    ...
    filterFeedback: function (feedbackList, pageSize, pageNumber) {
      if (this.activeCategory) {
        feedbackList = feedbackList.filter(
          (feedback) => feedback.category === this.activeCategory
        );
      }
      this.feedbackStates[1].count = (feedbackList?.filter(
        (feedback) => !feedback.state.toLowerCase().includes("close")
      )).length;
      this.feedbackStates[2].count = feedbackList.filter((feedback) =>
        feedback.state.toLowerCase().includes("close")
      ).length;
      if (this.selectedState) {
        if (this.selectedState.includes("close")) {
          ...
          !this.selectedState.includes("close") &&
          !this.selectedState.includes("all")
        ) {
          feedbackList = feedbackList.filter(
            (feedback) => !feedback.state.toLowerCase().includes("close")
          );
      ...
      if (this.selectedModule && this.selectedModule !== "Any") {
        feedbackList = feedbackList.filter(
          (feedback) => feedback.module === this.selectedModule
        );
      }
      if (this.searchText) {
        ...
        feedbackList = feedbackList.filter((feedback) => {
          return (
            ...
            !!(
              feedback?.createdBy &&
              (regex.test(feedback?.createdBy?.name) ||
                regex.test(feedback?.createdBy?.uid))
            ) ||
            !!(
              feedback?.assignee &&
              (regex.test(feedback?.assignee?.name) ||
                regex.test(feedback?.assignee?.email) ||
                regex.test(feedback?.assignee?.url))
            )
          );
        });
      }
      feedbackList = feedbackList.slice(
        Number(pageNumber),
