...
function processIntegrationInput(userFeedback: any, app: any, userData: any) {
...
async function createGithubIssue(feedbackConfig: any, userFeedback: any, app: any, userData: any) {
  ...
  const githubResponse = await axios.request<any>({
  ...
  }).then((response) => response.data.data.createIssue).catch((err: Error) => {
    logger.error(err);
...
async function createGitlabIssue(feedbackConfig: any,
  userFeedback: any, app: any, userData: any) {
  const processedFeedback = processIntegrationInput(userFeedback, app, userData);
  const params = {
    gitlabIssueInput: {
      title: processedFeedback.summary,
