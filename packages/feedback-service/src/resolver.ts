      ...
      return Feedback.find()
        ...
        .then((response: FeedbackType[]) => {
          return response.map((res: any) => {
            const filteredApps =
              Apps.filter((app: any) => app?.id === res?.config)[0] || null;
          ...
          if (feedbackList.length) {
            ...
            Object.keys(feedback).map((key: string) => {
              feedback[key].forEach((groupedList: FeedbackType, index: any) => {
                timestampQuery += `
                rhatUUID_${(groupedList.createdBy as string).replace(
                  /-/g,
                  ""
                )}:getUsersBy(rhatUUID:"${groupedList.createdBy}") {
                  cn
                  mail
                  uid
                  rhatUUID
                ...
                ${
                  groupedList.updatedBy
                    ? `
                rhatUUID_${(groupedList.updatedBy as string).replace(
                  /-/g,
                  ""
                )}:getUsersBy(rhatUUID:"${groupedList.updatedBy}") {
                  cn
                  mail
                  uid
                  rhatUUID
                ...
                if ((groupedList as any).feedback.sourceType === "GITLAB") {
                  query += `
                        gitlab_${index}_${
                    groupedList.ticketUrl.split("/")[
                      groupedList.ticketUrl.split("/").length - 1
                    ]
                  }:project(fullPath: "${
                    (groupedList as any).feedback.projectKey
                  }") {
                          issue(iid: "${
                            groupedList.ticketUrl.split("/")[
                              groupedList.ticketUrl.split("/").length - 1
                  ...
                  (groupedList as any).feedback.sourceType === "GITHUB"
                ) {
                  query += `
                    gitlab_${index}_${
                    groupedList.ticketUrl.split("/")[
                    ...
                    groupedList.ticketUrl.split("/")[
                      groupedList.ticketUrl.split("/").length - 3
                    ]
                  }", owner: "${
                    groupedList.ticketUrl.split("/")[
                      groupedList.ticketUrl.split("/").length - 4
                    ]
                  }") {
                          issue(number: ${Number(
                            groupedList.ticketUrl.split("/")[
                  ...
                  (groupedList as any).feedback.sourceType === "JIRA"
                ) {
                  query += `${
                    groupedList.ticketUrl.split("/")[
                      groupedList.ticketUrl.split("/").length - 1
        ...
        .then(async () => {
          if (feedbackList.length) {
            ...
            userData = await FeedbackIntegrationHelper.getUserProfiles(
              timestampQuery
            );
            queryList.map(async (queries: any) => {
              if (queries.source === "GITLAB") {
                const gitlabPromise = new Promise(async (resolve, reject) => {
                  const gitlabResponse: Array<object> =
                    await FeedbackIntegrationHelper.listGitlabIssues(queries);
                ...
                const githubPromise = new Promise(async (resolve, reject) => {
                  const githubResponse: Array<object> =
                    await FeedbackIntegrationHelper.listGithubIssues(queries);
                ...
                queries.query
                  ...
                  .map((issue: any) => {
                    const jiraPromise = new Promise(async (resolve, reject) => {
                      ...
                      const jiraResponse: any =
                        await FeedbackIntegrationHelper.listJira(query);
  ...
  Mutation: {
    async createFeedback(root: any, args: any, ctx: any) {
      ...
      let userQuery = `query GetUsersBy {
        rhatUUID_${(args.input.createdBy as string).replace(
          /-/g,
          ""
        )}:getUsersBy(rhatUUID:"${args.input.createdBy}") {
          uid
          rhatUUID
          cn
          mail
      ...
      if (!args.input.config && args.input?.stackInfo?.path) {
        ...
        apps = apps.filter(
          (response: any) =>
            response.path === `/${args.input.stackInfo.path.split("/")[1]}`
        )[0];
        args.input.config = apps.id;
      } else if (
        (args.input.config && !args.input?.stackInfo?.path) ||
        (args.input.config && args.input?.stackInfo?.path)
      ) {
        let appParam = {
...
${
  args.input?.stackInfo?.stack
    ? `Stack - ${args.input?.stackInfo?.stack}<br/>`
    : ``
}
${
  args.input?.stackInfo?.path
    ? `URL - ${args.input?.stackInfo?.path}<br/><br/>`
    : ``
}
...
`;
      if (!args.input.description) {
        ...
        args.input.description =
          args.input.description.concat(descriptionTemplate);
      }
      let summary =
        !args?.input?.summary || args.input.summary.length > 230
          ? `${args.input.category} reported by ${userData[0].cn} for ${apps.name}`
          : args.input.summary;
      let description =
        args.input?.summary?.length > 230
          ? args.input.summary + args.input.description
          : args.input.description;
      ...
      switch (apps.feedback.sourceType) {
        case "GITHUB":
          ...
          const githubResponse =
            await FeedbackIntegrationHelper.createGithubIssue(query);
          apiResponse = {
            ...args.input,
        ...
        case "JIRA":
          ...
          const jiraResponse = await FeedbackIntegrationHelper.createJira(
            jiraQuery
          );
          apiResponse = {
            ...args.input,
            ticketUrl: `https://${
              apps.feedback.sourceApiUrl || process.env.JIRA_HOST
            }/browse/${jiraResponse.key}`,
          };
          break;
        case "GITLAB":
          ...
          const gitlabResponse =
            await FeedbackIntegrationHelper.createGitlabIssue(gitlabQuery);
          apiResponse = {
            ...args.input,
      ...
      const emailData = FeedbackIntegrationHelper.createEmailTemplate(
        userData,
        apiResponse,
        apps
      );
      FeedbackIntegrationHelper.sendEmail(emailData);
      apiResponse.description = apiResponse.description.replace(
        /(<([^>]+)>)/gi,
        ""
      );
      return new Feedback(apiResponse)
        .save()
        .then(async (response: any) => {
          response.createdBy = userData[0].cn;
          const formattedSearchResponse =
            FeedbackIntegrationHelper.formatSearchInput(response);
          FeedbackIntegrationHelper.manageSearchIndex(
          ...
          return Object.assign(response, args.input)
            ...
            .then(async (feedback: FeedbackType) => {
              let userQuery = `query ListUsers {
                rhatUUID_${(feedback.createdBy as string).replace(
                  /-/g,
                  ""
                )}:getUsersBy(rhatUUID:"${feedback.createdBy}") {
                  cn
                  uid
                  rhatUUID
                }
                ${
                  feedback.updatedBy
                    ? `
                rhatUUID_${(feedback.updatedBy as string).replace(
                  /-/g,
                  ""
                )}:getUsersBy(rhatUUID:"${feedback.updatedBy}") {
                  cn
                  uid
                  rhatUUID
                }
              ...
              `;
              let userData = await FeedbackIntegrationHelper.getUserProfiles(
                userQuery
              );
              response.createdBy = userData.filter(
                (user: any) => user.rhatUUID === feedback.createdBy
              )[0].cn;
              response.updatedBy = userData.filter(
                (user: any) => user.rhatUUID === feedback.updatedBy
              )[0].cn;
              const formattedSearchResponse =
                FeedbackIntegrationHelper.formatSearchInput(response);
        ...
        .then(async (response: any) => {
          if (response.length) {
            response.map((data: any) => {
              userQueryParams += `rhatUUID_${(data.createdBy as string).replace(
                /-/g,
                ""
              )}:getUsersBy(rhatUUID:"${data.createdBy}") {
                name
                title
                uid
                rhatUUID
              ...
              ${
                data.updatedBy
                  ? `
              rhatUUID_${(data.updatedBy as string).replace(
                /-/g,
                ""
              )}:getUsersBy(rhatUUID:"${data.updatedBy}") {
                name
                title
                uid
                rhatUUID
              ...
              const formattedData =
                FeedbackIntegrationHelper.formatSearchInput(data);
          ...
          query ListUsers {
            ...
            const indexStatus = searchInput.map((searchData: any) => {
              searchData.input.documents.createdBy = userData.filter(
                (user: any) =>
                  user.rhatUUID === searchData.input.documents.createdBy
              )[0]?.name;
              searchData.input.documents.updatedBy = userData.filter(
                (user: any) =>
                  user.rhatUUID === searchData.input.documents?.updatedBy
              )[0]?.name;
              return FeedbackIntegrationHelper.manageSearchIndex(
                searchData,
                "index"
              );
            });
