...
import { getTransporterInstance, getEnvironmentPaths  } from "./helpers";
...
export const FeedbackResolver = {
  Subscription: {
    // subscriptions
    updatedFeedbackData: {
      subscribe: () => pubsub.asyncIterator(Feedback_UPDATE)
    },
  },
  Query: {
    // queries
  ...
  Mutation: {
        ...
        Feedback.findById(response._id)
          ...
          .then(fb => {
            if (fb) {
              ...
              if (fb.iid) {
                imsg = "You can track updates for your feedback via Jira at: Need to paste Jira URL" + fb.iid;
              } else {
                imsg = "";
              }
              if (fb.description) {
              ...
              const environmentPaths: any = getEnvironmentPaths();

              if (environmentPaths.name === "production") {
                transporter.sendMail({
                  from: "noreply@redhat.com",
                  to: fb.timestamp.createdBy.name + " <" + fb.timestamp.createdBy.email + ">",
                  cc: "one-portal-devel@redhat.com",
                  subject: `[One Portal]: Thanks for ${feedbackType} on ${feedbackfor}`,
                  text: body,
    ...
    deleteFeedback(root: any, args: any, ctx: any) {
      return Feedback.findByIdAndRemove(args._id).then(response => response).catch(err => err);
    },
