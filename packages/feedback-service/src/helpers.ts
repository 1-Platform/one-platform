...
class FeedbackHelper {
  ...
  public createGitlabIssue(query: any) {
    ...
    return fetch(`${query.sourceUrl || process.env.GITLAB_API}`, {
      ...
      .then((response: any) => response.json())
      .then((result: any) => {
        result.data.createIssue.issue.labels =
          result?.data?.createIssue?.issue?.labels?.edges.map(
            (label: any) => label.node.title
          );
    ...
    return fetch(`${gitlabQuery.sourceUrl || process.env.GITLAB_API}`, {
      ...
      .then((response: any) => {
        return Object.keys(response.data).map((key) => {
          response.data[key].issue.labels = response.data[
            key
          ].issue.labels.edges.map((edge: any) => edge.node.title);
          response.data[key].issue.assignee = {
            name: response.data[key]?.issue?.assignees?.nodes[0]?.name || null,
            email:
              response.data[key]?.issue?.assignees?.nodes[0]?.email || null,
  ...
  public listJira(jiraQuery: any) {
    ...
    return JiraApiClient.searchJira(`issue = ${jiraQuery.query}`)
      .then((response) => {
...
`;
    const emailData = {
      ...
      subject: `${feedback.error ? feedback.error : feedback.experience} - ${
        feedback.category.charAt(0) +
        feedback.category.substring(1).toLowerCase()
