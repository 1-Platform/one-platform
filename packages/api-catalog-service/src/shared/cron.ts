...
export class NamespaceCron {
    public async checkAPIHash () {
        const namespaces: NamespaceType[] = await Namespace.find().lean();
        namespaces.map( ( namespace:any ) => {
            let category = namespace.category as ApiCategory;
            namespace.environments?.map( async ( environment: any ) => {
                namespace.lastCheckedOn = new Date();
                let hash = await apiCatalogHelper.manageApiHash( category, environment );
