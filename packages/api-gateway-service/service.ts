...
const subsciptionsBaseUrl =
  process.env.SUBSCRIPTIONS_BASE_URL ?? path.join(baseUrl, "/subscriptions");
          ...
          requestDidStart: (requestContext) => {
            ...
            console.log(
              ...
              `- Incoming ${
                ...
                requestContext.request.http?.headers.get("origin") || "unknown"
              }`,
              `- via ${requestContext.request.http?.headers.get("user-agent")}`
            );
          },
        },
      ],
...
export default server.listen(port, () => {
  console.log(
    `Gateway Running on ${process.env.NODE_ENV} environment at port ${port}`
  );
});
