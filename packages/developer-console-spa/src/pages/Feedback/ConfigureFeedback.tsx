...
} from 'common/services/feedbackConfig';
import Loader from '../../common/components/Loader';
import { ProjectContext } from 'common/context/ProjectContext';
import useFeedbackConfig from 'common/hooks/useFeedbackConfig';
import { requiredMsg } from 'common/utils/formErrorMsg';
...
function ConfigureFeedback() {
  /**
   * projectId: it is the unique slug generated for an app used in url
   * project.id: its db id of an app in app service. Used for reference in other services
   */
  const { projectId, loading: appLoading, project } = useContext(ProjectContext);
  const { setHeader } = useContext(HeaderContext);
  const {
    feedbackConfig,
    setFeedbackConfig,
    loading: feedbackConfigLoading,
  } = useFeedbackConfig(project.id);
  const {
    handleSubmit,
    control,
    watch,
  ...
  useEffect(() => {
    setHeader([
      { title: 'Feedback', path: `/${projectId}/feedback` },
      { title: 'Manage', path: `/${projectId}/feedback/edit` },
    ]);
    return () => setHeader([]);
  ...
  const saveFeedbackConfig = async (formData: FormData) => {
    const data = { ...formData, appId: project.id };
    try {
      if (isUpdate) {
      ...
      history.push(`/${projectId}/feedback`);
    } catch (error) {
