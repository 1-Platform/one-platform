...
function ConfigureCouchDB ( props: any ) {
  ...
  const filteredDBs = app.database.databases.filter( ( db: string ) => regex.test( db ) );
                ...
                :
                  <Gallery className="lighthouse-projectForm--marginTop"
                    ...
                    } }>
                    {
                        filteredDBs
                          .map( ( db: string ) => {
                          return <GalleryItem key={ db }>
                            <Card className="couch-db--card">
                              <CardHeader>
                                <CardActions>
                                  <Dropdown
                                    onSelect={ onCardMenuSelect }
                                    toggle={ <KebabToggle className="" onToggle={ () => { onCardMenuToggle(db) } } /> }
                                    isOpen={ expandedMenuCardId ===  db }
                                    isPlain
                              ...
                              <CardFooter>
                                <Button variant="link" onClick={ () => openFauxtonGUI( db ) }>Open Fauxton GUI &nbsp; <ExternalLinkAltIcon/></Button>
                              </CardFooter>
                            </Card>
                          </GalleryItem>
                        } )
