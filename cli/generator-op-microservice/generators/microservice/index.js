...
module.exports = class extends Generator {
  prompting() {
    ...
    this.log(
      yosay(
        `Welcome to the One Platform Microservice generator ${chalk.red(
          "generator-op-microservice"
        )}!`
      )
    );
    ...
    return this.prompt(prompts).then((answers) => {
      ...
      const hyphenatedName =
        answers.name.indexOf(" ") !== -1
          ? answers.name.replace(/\s+/g, "-").toLowerCase()
          : answers.name;
      const underscoredName =
        answers.name.indexOf(" ") !== -1
          ? answers.name.replace(/\s+/g, "_").toUpperCase()
          : answers.name.toUpperCase();
      const capitalizedName = _.chain(answers.name)
        .camelCase()
        .upperFirst()
        .value();
      ...
      graphqlSupport =
        this.options.type === "graphql" || answers.type === "graphql";
      dbSupport = answers.dbSupport;
      const packageName = graphqlSupport
        ? `${hyphenatedName}-service`
        : `${hyphenatedName}-integration`;
      this.props = {
  ...
  writing() {
    ...
    if (graphqlSupport) {
      if (fs.existsSync(this.templatePath("gql.service.ts"))) {
        this.fs.copyTpl(
          ...
          this.destinationPath(`${this.props.packageName}/service.ts`),
          this.props
        );
      }
      ...
      if (fs.existsSync(this.templatePath("src/resolver.ts"))) {
        this.fs.copyTpl(
          ...
          this.destinationPath(`${this.props.packageName}/src/resolver.ts`),
          this.props
        );
      }
      ...
      if (fs.existsSync(this.templatePath("src/typedef.graphql"))) {
        this.fs.copyTpl(
          ...
          this.destinationPath(`${this.props.packageName}/src/typedef.graphql`),
          this.props
        );
      }
      ...
      if (fs.existsSync(this.templatePath("src/api.ts"))) {
        this.fs.copyTpl(
          ...
          this.destinationPath(`${this.props.packageName}/src/api.ts`),
          this.props
        );
      }
    ...
    if (fs.existsSync(this.templatePath("src/types.d.ts"))) {
      this.fs.copyTpl(
        ...
        this.destinationPath(`${this.props.packageName}/src/types.d.ts`),
        this.props
      );
    }
    if (dbSupport) {
      if (fs.existsSync(this.templatePath("src/schema.ts"))) {
        this.fs.copyTpl(
          ...
          this.destinationPath(`${this.props.packageName}/src/schema.ts`),
          this.props
        );
      }
    }
    if (fs.existsSync(this.templatePath("src/helpers.ts"))) {
      this.fs.copy(
        ...
        this.destinationPath(`${this.props.packageName}/src/helpers.ts`)
      );
    }
    if (fs.existsSync(this.templatePath("src/config/config.ts"))) {
      this.fs.copyTpl(
        this.templatePath("src/config/config.ts"),
        this.destinationPath(`${this.props.packageName}/src/config/config.ts`),
        this.props
      );
    }
