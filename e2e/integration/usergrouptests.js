context("Test for UserGroup", () => {
  ...
  it("Test for top header section", () => {
    cy.get(".app").within(() => {
      cy.get(".opc-header").within(() => {
        cy.get(".opc-header__top-row--header-name").should(
          "contain.text",
          "User Group SPA"
        );
      });
  ...
  it("Test for search user group by name", () => {
    ...
    cy.get("tbody").within(() => {
      cy.get('[data-label="Group Name"]')
        .first()
        .should("have.text", "DSAL Admins");
    });
  });
  ...
  it("Test for my profile name", () => {
    ...
    cy.get(".pf-c-toolbar__item").should(
      "have.text",
      "one-portal-test sso-tester"
    );
  });
  ...
  it("Test for my profile Email", () => {
    cy.get(".pf-c-description-list__group")
      ...
      .within(() => {
        ...
        cy.get(".pf-c-description-list__description").should(
          "have.text",
          "one-portal-test@redhat.com"
        );
      });
  });
  ...
  it("Test for my profile Kerberos ID", () => {
    cy.get(".pf-c-description-list__group")
      .eq(2)
      .within(() => {
        cy.get(".pf-c-description-list__term").should(
          "have.text",
          "Kerberos ID"
        );
        cy.get(".pf-c-description-list__description").should(
  ...
  it("Test for my profile Authorization Section", () => {
    cy.get(".pf-c-accordion").should(
      "have.text",
      "Authorization & API PermissionsUnder Development"
    );
  });
});
